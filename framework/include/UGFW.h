...
#pragma region SColor structure to describe a colour
	...
	**/
	struct SColour
	{
		...
		SColour(unsigned int a_iColour) { rgba.colour = a_iColour; }
...
#pragma region Line Drawing functionality
	...
	/// @param a_thickness the default thickness of the line, default is 1.0f
	/// @param a_layer the layer that the line appears on layer 0 is the back layer, higher numbers move the layer forwards
	/// @param a_Color the colour of the line.
	UGFWAPI void			DrawLine(float a_iStartX, float a_iStartY, float a_iEndX, float a_iEndY, float a_thickness = 1.f, unsigned int a_layer = 0.f, SColour a_Colour = SColour(0xFF, 0xFF, 0xFF, 0xFF));

#pragma endregion
#pragma region Input handling functionality
	/// @brief Function returns weather a key has just been pressed this frame
	/// @param a_uiKey the unique identifier for the key, these are kept in an enum referenced from GLFW
	/// @return boolean value to represent whether a key has just been pressed
	UGFWAPI bool			WasKeyPressed(unsigned int a_uiKey);
	/// @brief Function returns weather a key has just been released
	/// @param a_uiKey the unique identifier for the key, these are kept in an enum referenced from GLFW
	/// @return boolean value to represent whether the key has just been released
	UGFWAPI bool			WasKeyReleased(unsigned int a_uiKey);
	/// @brief Function returns weather a key is currently being pressed
	/// @param a_uiKey the unique identifier for the key, these are kept in an enum referenced from GLFW
	/// @return boolean value to represent whether a key is held down or not
	UGFWAPI bool			IsKeyDown(unsigned int a_uiKey);
	/// @brief Function returns weather a key is currently not pressed
	/// @param a_uiKey the unique identifier for the key, these are kept in an enum referenced from GLFW
	/// @return boolean value to represent whether a key is not being pressed
	UGFWAPI bool			IsKeyUp(unsigned int a_uiKey);
	...
	UGFWAPI void			GetMousePos(int& a_uiMousePosX, int& a_uiMousePosY);
	...
	UGFWAPI bool			IsMouseButtonDown(int a_iMouseButton);
	/// @brief Function to test if a mouse button is being pressed. 
	/// @param a_iMouseButton and integer identifier to identify the button to test for
	/// @return boolean value indicating if button is not being pressed
	UGFWAPI bool			IsMouseButtonUp(int a_iMouseButton);
	...
	UGFWAPI bool			WasMouseButtonReleased(int a_iMouseButton);
	/// @brief Function to Test to see if the mouse button has been released
	/// @param a_iMoustButton an integer identifier to identify the button to test for release
	/// @return boolean value to indicate weather button has just been presseed.
	UGFWAPI bool			WasMouseButtonPressed(int a_iMouseButton);
#pragma endregion
#pragma region Sprite creation and mainpulation functionality
	...
	UGFWAPI void			SetSpritePosition(int a_uiSpriteID, float a_fXPos, float a_fYPos);
	/// This funciton moves a Sprite
	/// @param a_uiSpriteID the ID of the sprite to be moved
	/// @param a_fVec a float vector that contains two values (X & Y) to relocate the sprite to
	UGFWAPI void			SetSpritePosition(int a_uiSpriteID, const float* a_fVec);
	...
	UGFWAPI void			SetSpriteRotation(int a_uiSpriteID, float a_fRotation);
	/// @brief Function to rotate a sprite by a_fRotation number of degrees, positive rotation is in a clockwise direction
	/// @param a_uiSpriteID the unique ID for the sprite
	/// @param a_fRotation the amount in degrees to rotate the sprite by, this value is cumulative E.G passing in a rotation value of 1 Degree every frame will incrementally rotate the sprite each frame
...
#pragma region Font Rendering functionality
	...
	UGFWAPI void			DrawString(const char* a_pText, int a_iXPos, int a_iYPos, unsigned int a_layer, SColour a_sColour = SColour(0xFF, 0xFF, 0xFF, 0xFF));

	UGFWAPI int				AddFont(const char* a_pFontName, unsigned short a_size);
	UGFWAPI void			SetFont(const int& a_fontID);
	UGFWAPI void			RemoveFont(const int& a_fontID);
